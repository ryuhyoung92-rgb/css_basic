...
4. `tag {}` 우선순위 낮음
## CSS 본인 적용과 상속 적용 차이
* 자식이 2개 이상일 때 부모에 속성을 정용하는 것으로 자식에게 공통 값을 적용할 수 있다.
...
## 글꼴 사용 시 주의사항
* 모든 컴퓨터 기본글꼴(고딕, 궁서, 바탕 등)이 아니라면 반드시 웹글꼴을 사용하여 웹접근성을 높여줘야 함.
...
* `display:;` : 요소의 형태유형 변경속성
    ...
    * inline 변경가능요소 : h1, p, address 등 블록태그
        * `h1 {display:inline;}`
        * 내용만큼만 크기 인식
        * 옆으로 나열
    * block 변경가능요소 : a, span, em, del 등 인라인태그
        * `a {display:block;}`
        * 크기와 여백값을 인식
...
* `margin` : 바깥쪽(형제 사이) 여백(피그마 기준 **간격**)
    ...
    * `margin:10px 20px 30px 40px;` 상/우/하/좌(시계방향으로 읽기)
...
* `table {width:300px}` 열이 3개일 경우 100씩 자동 분배
    ...
    * (열 안 내용 크기에 따라 달라짐)
* `width, height, padding` 속성은 같은 수평/수직 방향에 해당하는 열과 함께 적용됨.
...
### button
* `<button type="button" 이벤트="자바스트립트명령어작성">`
...
## 웹브라우저 별 스크롤 디자인(그대로 복사해 사용하기) - 색깔은 같은 색상끼리 통일해 사용하기
/* Firefox */
...
* `:hover` 선택자로 인해 기존속성에서 다른속성으로 변경되는 CSS에 애니메이션을 주고싶을 때
    ...
    * 아래 연결 동작) 변경된 color만 transition 시 변경됨
    * `선택자 {tranition:color 1s; color:#000; font-size:1rem;}`
    * 문법 : `transition:변경속성 지속시간 속도 지연시간`
    * `transition:color 1s linear 2s`
    * 2초 기다리고 1초 동안 일정속도로 color속성에 애니 적용
## animation + @keyframes
### 적용 순서
...
* `keyframes + animation`은 페이지로딩 시 등장 애니메이션, 스크롤 내릴 시 나타나는 콘텐츠의 등장 애니메이션 등에 사용함(사용자 hover 동작과 대부분 관계없음)
### 명령 해석
